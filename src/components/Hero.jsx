...
const Hero = () => {
    ...
    return (
        <div className="flex w-[90%] gap-5 m-auto h-full max-tablet:flex-col max-tablet:gap-0">
            ...
            <div className="w-[50%] p-2 flex flex-col gap-10 mt-24 max-mobile:mt-20 max-tablet:w-full">
                {itemOneRender.map(hero => (
                    <div key={hero.id}>
                        <h3 className="bg-btn w-fit rounded-full desktop:px:5 desktop:py-3 px-3 py-1 max-mobile:text-xs max-mobile:px-5 max-mobile:py-3 max-mobile:w-48" >{hero.content}</h3>
                    </div>
                ))}

                {itemSecondRender.map(hero => (
                    <div key={hero.id}>
                        <h1 className="text-6xl desktop:text-8xl max-mobile:text-4xl max-mobile:w-[22rem] font-semibold">{hero.content}</h1>
                    </div>
                ))}
                ...
                <div className="flex gap-10 max-mobile:gap-2">
                    {itemThirdRender.map(hero => (
                        <div key={hero.id}>
                            <p className="btn-primary desktop:text-2xl max-mobile:text-sm flex items-center max-mobile:w-[10rem] gap-3">{hero.content}<FaArrowUpRightFromSquare/></p>
                        </div>
                ))}
                    {itemFourRender.map(hero => (
                        <div key={hero.id}>
                            <p className="btn-secondary desktop:text-2xl max-mobile:text-sm flex items-center max-mobile:w-[10rem] gap-1"><FaSquarePhone/>{hero.content}</p>
                        </div>
                ))}
                </div>
            </div>

            <div className="w-[50%] max-tablet:w-full h-fit relative mt-24">
                ...
                <div className="px-4 desktop:px-6 absolute bottom-0 shadow-xl left-0 border-8 border-transparent py-4 desktop:py-6 rounded-xl bg-white flex flex-col desktop:w-[30%] w-[35%] max-mobile:w-[60%] gap-4 ">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <p className="text-xs desktop:text-base font-semibold">Review 4.8/5.0</p>
                        </div>
                    </div>
                </div>
            </div>
