  ...
  return (
    <div className="flex items-end max-tablet:flex-col gap-4 w-full p-14 max-tablet:p-5 max-tablet:gap-10 max-mobile:gap-5">
        ...
        <div>
          {Testimonials.map(testimonial => (
            <div key={testimonial.id} className="flex flex-col gap-6">
              <h3 className="text-t-primary font-semibold">{testimonial.heading}</h3>
              <h1 className="text-4xl max-tablet:text-3xl max-mobile:text-2xl font-semibold">{testimonial.title}</h1>
            </div>
          ))}
        </div>
      ...
      <div className="grid max-mobile:grid-flow-row grid-cols-4 gap-4 grid-rows-6 grid-flow-col w-[100%] h-[32rem]">
        <div className="col-span-2 max-mobile:hidden row-span-4 rounded-3xl overflow-hidden"><img src={codingImage} alt="" className="w-full h-full object-cover"/></div>
        ...
        <div className="col-span-2 max-mobile:col-span-4 row-span-4 rounded-3xl overflow-hidden shadow-xl"><img src={digitalImage} alt="" className="w-full h-full object-cover"/></div>
      </div>
    </div>
  )
}
