...
const Blog = () => {
  ...
  return (
    ...
    <div className="bg-blue-50 max-mobile:px-6 max-mobile:py-10 py-20">
      {
        Blogs.map((blog, index) => (
          <div key={index} className="list-none m-auto flex flex-col items-center justify-center gap-2">
            <h2 className="text-t-primary font-medium text-sm">{blog.heading}</h2>
            <p className="font-semibold text-2xl">{blog.title}</p>
            <div className="flex items-center max-mobile:flex-col justify-center gap-4 ">
              <div className=" w-[30%] max-mobile:w-full flex flex-col gap-4">
              ...
              <div className="bg-b-primary h-[120%] max-mobile:w-full w-[60%] rounded-lg">
