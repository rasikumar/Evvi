      ...
      <div className={`flex flex-col tablet:flex-row items-center ${isOpen ? 'absolute items-center justify-center left-0 top-0 -z-10 h-screen w-full p-5 gap-7 bg-white block transition delay-200' : 'hidden'} tablet:block `}>
        {
          Navigation.map(navigate => (
            <a key={navigate.id} href={navigate.url} onClick={closeMenu}  className={`text-t-primary mr-7 font-medium hover:border-b-2 hover:text-t-secondary border-t-secondary desktop:text-2xl laptop:text-sm  ${isOpen ? 'ml-7' : ''}`}>
              {navigate.title}
            </a>
          ))
        }
